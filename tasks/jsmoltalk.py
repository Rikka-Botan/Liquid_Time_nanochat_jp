...
class JSmolTalk(Task):
    """ smol-smoltalk dataset. train is 132K rows, test is None. """

    def __init__(self, split, **kwargs):
        super().__init__(**kwargs)
        assert split in ["train", "test"], "SmolTalk split must be train|test"
        self.ds = load_dataset("RikkaBotan/Cute_Synthetic_smoltalk_jp_sft" , split=split)
        self.length = len(self.ds["messages"])

    def num_examples(self):
        return self.length

    def get_example(self, index):
        row = self.ds["messages"][index]
        messages = row
        # ---------------------------------------------------------------------
